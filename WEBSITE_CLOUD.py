...
with st.sidebar:
    ...
    st.title("🌿 Macrofitas - Aquasmart")
    st.markdown("---")
    mostrar_qa = st.toggle("💧 Qualidade da Água", value=False)
...
# =====================================================================
# QUALIDADE DA ÁGUA (PLACEHOLDER)
# =====================================================================
if mostrar_qa:
    st.markdown("## 💧 Qualidade da Água")
    st.info("Seção reservada para indicadores de qualidade da água. Adicione aqui os parâmetros desejados (ex: turbidez, clorofila, temperatura, OD etc.).")
    st.markdown("---")
...
file_selected = os.path.join(
    classif_folder,
    f"classificado_macrofitas_{selected_date}.tif"
)
...
fig_path = os.path.join(
    output_vis_folder,
    f"fig_macrofitas_{selected_date}.png"
)
...
df_y1 = df_area[df_area['Data'].dt.year == year1] \
    .groupby(df_area['Data'].dt.month) \
    .mean(numeric_only=True)

df_y2 = df_area[df_area['Data'].dt.year == year2] \
    .groupby(df_area['Data'].dt.month) \
    .mean(numeric_only=True)
